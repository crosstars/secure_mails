...
class Ui_Login_win(object):
    def setupUi(self, Login_win ,login_info=None,ref=None):
        ...
        self.login_btn = QtWidgets.QPushButton(self.login_grp, enabled=False)
    ...
    def retranslateUi(self, Login_win):
        ...
        self.uname.setWhatsThis(_translate(
            "Login_win", "<html><head/><body><p>Username</p></body></html>"))
        self.label_3.setText(_translate("Login_win", "Username (Email id)"))
        self.error.hide()

    def login_function(self):

        User = ins.current_users(self.ref , self.uname.text() , self.passwd.text() ,self.ip)
        if User.db_is_user():
            if not User.check(self):
                self.next=3
                print("login_UNsuccesful")
                #error
            else :
                self.next=4
                self.login_info = User.checkdict
    ...
    def enable_btn(self):
        self.login_btn.setEnabled(
            self.passwd.text() != "" and self.uname.text() != "")
